        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    cv2.rectangle(
                        frame, (contx, conty), (contx + contw, conty + conth), (0, 0, 255), 2)

                    # finding top of tape
                    try:
                        list = []

                        # finding y values in the tape, within the top 15
                        # pixels of the frame
                        for var in c:
                            if var.item(1) < 15:
                                list.append(var)
                    ...
                    # print(self.angle)
                    # print(self.distance)
                    self.pipeAngleWrite.put(self.angle)
                    self.pipeDistanceWrite.put(self.distance)
                    # cv2.imshow("frame", frame)
                    # cv2.waitKey(1)
